...
import { getUser, setNsfwMode, setImageCount, setOrientation, setAnimationMode } from "../../db/queries.js";
...
export function registerSettings(bot: any) {
  ...
  bot.callbackQuery(/^set_nsfw:(.+)$/, async (ctx: Context) => {
    ...
    const userId = ctx.from?.id;
    if (!userId) return;
    if (["sfw", "nsfw", "any"].includes(value)) await setNsfwMode(userId, value);
    await showSettings(ctx, userId);
  });
