...
export async function setBanned(
  ...
  runSql("UPDATE users SET is_banned = ? WHERE telegram_id = ?", [
    banned ? 1 : 0,
    telegramId,
  ]);
}
...
export async function setNsfwMode(
  ...
  runSql("UPDATE users SET nsfw_mode = ? WHERE telegram_id = ?", [
    mode,
    telegramId,
  ]);
}
...
export async function setLanguage(
  ...
  runSql("UPDATE users SET language = ? WHERE telegram_id = ?", [
    lang,
    telegramId,
  ]);
}
...
export async function logCommand(
  ...
  runSql("INSERT INTO commands_used (user_id, command) VALUES (?, ?)", [
    telegramId,
    command,
  ]);
}
...
export async function addFavorite(
  ...
  runSql("INSERT OR IGNORE INTO favorites (user_id, image_id) VALUES (?, ?)", [
    telegramId,
    imageId,
  ]);
}
...
export async function removeFavorite(
  ...
  runSql("DELETE FROM favorites WHERE user_id = ? AND image_id = ?", [
    telegramId,
    imageId,
  ]);
}
...
export async function setImageCount(
  ...
  runSql("UPDATE users SET image_count = ? WHERE telegram_id = ?", [
    count,
    telegramId,
  ]);
}
...
export async function setOrientation(
  ...
  runSql("UPDATE users SET orientation = ? WHERE telegram_id = ?", [
    orientation,
    telegramId,
  ]);
}
...
export async function setAnimationMode(
  ...
  runSql("UPDATE users SET animation_mode = ? WHERE telegram_id = ?", [
    mode,
    telegramId,
  ]);
}
...
export async function toggleDailySubscription(
  ...
  runSql("UPDATE users SET daily_subscribed = ? WHERE telegram_id = ?", [
    newState,
    telegramId,
  ]);
  return newState === 1;
...
export async function addToAlbum(
  ...
  runSql("INSERT INTO album_images (album_id, image_id) VALUES (?, ?)", [
    albumId,
    imageId,
  ]);
  return true;
...
export async function removeFromAlbum(
  ...
  runSql("DELETE FROM album_images WHERE album_id = ? AND image_id = ?", [
    albumId,
    imageId,
  ]);
}
...
export async function setSendMode(
  ...
  runSql("UPDATE users SET send_mode = ? WHERE telegram_id = ?", [
    mode,
    telegramId,
  ]);
}
