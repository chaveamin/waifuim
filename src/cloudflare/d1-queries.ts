...
export async function upsertUser(
  ...
  const existing = await q1<{ telegram_id: number }>(
    "SELECT telegram_id FROM users WHERE telegram_id = ?",
    [telegramId],
  );
  if (existing) {
    await run(
      "UPDATE users SET username = ?, first_name = ?, last_name = ?, last_active = datetime('now') WHERE telegram_id = ?",
      [username, firstName, lastName, telegramId],
    );
  } else {
    let adminId = 0;
    try {
      adminId = Number((globalThis as any).ADMIN_TELEGRAM_ID ?? 0);
    } catch {}
    const isAdmin = telegramId === adminId ? 1 : 0;
    await run(
      "INSERT INTO users (telegram_id, username, first_name, last_name, is_admin) VALUES (?, ?, ?, ?, ?)",
      [telegramId, username, firstName, lastName, isAdmin],
    );
  }
}
...
export async function setBanned(
  ...
  await run("UPDATE users SET is_banned = ? WHERE telegram_id = ?", [
    banned ? 1 : 0,
    telegramId,
  ]);
}

export async function setNsfwMode(
  telegramId: number,
  mode: string,
): Promise<void> {
  await run("UPDATE users SET nsfw_mode = ? WHERE telegram_id = ?", [
    mode,
    telegramId,
  ]);
}
...
export async function setLanguage(
  ...
  await run("UPDATE users SET language = ? WHERE telegram_id = ?", [
    lang,
    telegramId,
  ]);
}
...
export async function logCommand(
  ...
  await run("INSERT INTO commands_used (user_id, command) VALUES (?, ?)", [
    telegramId,
    command,
  ]);
}
...
export async function getUserCommandCount(telegramId: number): Promise<number> {
  const row = await q1<{ count: number }>(
    "SELECT COUNT(*) as count FROM commands_used WHERE user_id = ?",
    [telegramId],
  );
  return row?.count ?? 0;
}
...
export async function addFavorite(
  ...
  await run(
    "INSERT OR IGNORE INTO favorites (user_id, image_id) VALUES (?, ?)",
    [telegramId, imageId],
  );
}
...
export async function removeFavorite(
  ...
  await run("DELETE FROM favorites WHERE user_id = ? AND image_id = ?", [
    telegramId,
    imageId,
  ]);
}
...
export async function isFavorited(
  ...
  const row = await q1(
    "SELECT 1 FROM favorites WHERE user_id = ? AND image_id = ?",
    [telegramId, imageId],
  );
  return !!row;
}
...
export async function getUserFavorites(
  ...
  limit: number = 20,
  offset: number = 0,
): Promise<{ image_id: number; created_at: string }[]> {
  return qAll(
...
export async function setImageCount(
  telegramId: number,
  count: number,
): Promise<void> {
  await run("UPDATE users SET image_count = ? WHERE telegram_id = ?", [
    count,
    telegramId,
  ]);
}
...
export async function setOrientation(
  ...
  await run("UPDATE users SET orientation = ? WHERE telegram_id = ?", [
    orientation,
    telegramId,
  ]);
}

export async function setAnimationMode(
  telegramId: number,
  mode: string,
): Promise<void> {
  await run("UPDATE users SET animation_mode = ? WHERE telegram_id = ?", [
    mode,
    telegramId,
  ]);
}

export async function toggleDailySubscription(
  telegramId: number,
): Promise<boolean> {
  const user = await getUser(telegramId);
  const newState = user?.daily_subscribed !== 1 ? 1 : 0;
  await run("UPDATE users SET daily_subscribed = ? WHERE telegram_id = ?", [
    newState,
    telegramId,
  ]);
  return newState === 1;
...
export async function setDailyTime(
  telegramId: number,
  hour: number,
  minute: number,
): Promise<void> {
  await run(
    "UPDATE users SET daily_hour = ?, daily_minute = ? WHERE telegram_id = ?",
    [hour, minute, telegramId],
  );
}

export async function markDailySent(telegramId: number): Promise<void> {
  await run(
    "UPDATE users SET last_daily_sent = datetime('now') WHERE telegram_id = ?",
    [telegramId],
  );
}
...
export async function getDailySubscribers(): Promise<DbUser[]> {
  return qAll<DbUser>(
    "SELECT * FROM users WHERE daily_subscribed = 1 AND is_banned = 0",
  );
}
...
export async function createAlbum(
  ...
  await run(
    "INSERT INTO albums (user_id, name, description, share_token) VALUES (?, ?, ?, ?)",
    [telegramId, name, description, token],
  );
...
export async function getUserAlbums(telegramId: number): Promise<any[]> {
  return qAll(
    `SELECT a.id, a.name, a.description, a.share_token, a.created_at, (SELECT COUNT(*) FROM album_images WHERE album_id = a.id) as image_count FROM albums a WHERE a.user_id = ? ORDER BY a.created_at DESC`,
    [telegramId],
  );
}
...
export async function addToAlbum(
  ...
  await run("INSERT INTO album_images (album_id, image_id) VALUES (?, ?)", [
    albumId,
    imageId,
  ]);
  return true;
...
export async function removeFromAlbum(
  ...
  await run("DELETE FROM album_images WHERE album_id = ? AND image_id = ?", [
    albumId,
    imageId,
  ]);
}
...
export async function getAlbumImages(
  ...
  return qAll(
    "SELECT image_id, added_at FROM album_images WHERE album_id = ? ORDER BY added_at DESC LIMIT ? OFFSET ?",
...
export async function isImageInAlbum(
  albumId: number,
  imageId: number,
): Promise<boolean> {
  const row = await q1(
...
export async function getUserAlbumsContainingImage(
  telegramId: number,
  imageId: number,
): Promise<any[]> {
  return qAll(
    "SELECT a.id, a.name FROM albums a INNER JOIN album_images ai ON ai.album_id = a.id WHERE a.user_id = ? AND ai.image_id = ?",
    [telegramId, imageId],
  );
}
...
export async function getMostActiveUsers(limit: number = 10): Promise<any[]> {
  return qAll(
    "SELECT u.telegram_id, u.username, u.first_name, COUNT(c.id) as command_count FROM users u INNER JOIN commands_used c ON c.user_id = u.telegram_id WHERE u.is_banned = 0 GROUP BY u.telegram_id ORDER BY command_count DESC LIMIT ?",
    [limit],
  );
}

export async function getMostFavoritedUsers(
  limit: number = 10,
): Promise<any[]> {
  return qAll(
    "SELECT u.telegram_id, u.username, u.first_name, COUNT(f.id) as fav_count FROM users u INNER JOIN favorites f ON f.user_id = u.telegram_id WHERE u.is_banned = 0 GROUP BY u.telegram_id ORDER BY fav_count DESC LIMIT ?",
    [limit],
  );
}
...
export async function getMostAlbumCreators(limit: number = 10): Promise<any[]> {
  return qAll(
    "SELECT u.telegram_id, u.username, u.first_name, COUNT(a.id) as album_count FROM users u INNER JOIN albums a ON a.user_id = u.telegram_id WHERE u.is_banned = 0 GROUP BY u.telegram_id ORDER BY album_count DESC LIMIT ?",
    [limit],
  );
}
...
export async function getDailySubscribersCount(): Promise<number> {
  const row = await q1<{ count: number }>(
    "SELECT COUNT(*) as count FROM users WHERE daily_subscribed = 1 AND is_banned = 0",
  );
  return row?.count ?? 0;
}

export async function setSendMode(
  telegramId: number,
  mode: string,
): Promise<void> {
  await run("UPDATE users SET send_mode = ? WHERE telegram_id = ?", [
    mode,
    telegramId,
  ]);
}
